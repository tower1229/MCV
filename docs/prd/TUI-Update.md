**实现状态（2026-08-05）：历史方案，已被后续 0.3 CLI 收缩取代。** 当前用户与运行契约只以 `README.md`、主 PRD 和 `TUI-Spec.md` 为准。下文的全局 Shell、deep-link、Init 后 Capture、旧测试数量和本地路径均只记录当时的设计背景，不代表当前仓库状态，也不得作为新实现或验收依据。
...
- Capture 已经过原值保留、路径参数化、冲突处理、预览、确认后才写入。
...
它们的接口只接收数据、返回数据，不输出、不询问用户。复杂的路径参数化、冲突处理、备份和事务写入继续隐藏在模块内部。
...
- 所有业务子命令都是同一 TUI Shell 的深链接，不是独立应用。从 `mcv` 首页或子命令进入的写操作，完成后都停留在结果页：按 `↑` / `↓` 滚动，按 `Enter` 或 `←` 返回已刷新的首页，按 `q` 直接退出。
- `status` 和 `discover` 等只读页面按 `↑` / `↓` 滚动，按 `←` 或 `Esc` 返回首页，按 `q` 直接退出。Restore review 按 `↑` / `↓` 浏览文件影响、按 `→` 打开焦点详情、按 `←` 关闭详情或返回首页，最终 Apply 仍必须按 `Enter`。其他多步流程中 `Esc` 返回上一步；在首页按 `Esc` 或 `q` 退出。
