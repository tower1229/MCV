...
import { deriveAssetCatalog, type AssetCatalogItem } from '../assets/catalog.js';
...
export function readAssets(
  ...
  try {
    ...
    const state: ContinuationState = decoded ?? {
    ...
    if (decoded && decoded.catalogRevision !== catalog.revision) {
      return readError(
        'mcp.staleCursor',
        'The Asset Catalog changed after this read_assets cursor was created. Restart the read with assetIds.',
      );
    }
    ...
    if (missing.length > 0) {
      return readError(
        'mcp.unknownAssets',
        `Unknown Asset IDs: ${missing.join(', ')}`,
      );
    }
...
function readPage(
  repositoryPath: string,
  initialState: ContinuationState,
  byId: ReadonlyMap<string, AssetCatalogItem>,
): ReadAssetsOutput {
  let state = initialState;
  let assets: AssetContent[] = [];
  ...
  while (state.assetIndex < state.assetIds.length) {
    ...
    const partial = largestFittingSlice(
      assets,
      asset,
      file.path,
      buffer,
    ...
    if (!partial) return finishPageOrError(assets, state);
...
function largestFittingSlice(
  assets: AssetContent[],
  asset: AssetCatalogItem,
  filePath: string,
  buffer: Buffer,
  state: ContinuationState,
): { assets: AssetContent[]; state: ContinuationState } | undefined {
  let low = 1;
  let high = buffer.byteLength - state.byteOffset - 1;
  let best: { assets: AssetContent[]; state: ContinuationState } | undefined;
...
function finishPageOrError(
  assets: AssetContent[],
  state: ContinuationState,
): ReadAssetsOutput {
  if (assets.length > 0) return finalizeReadOutput(assets, state);
  return readError(
    'mcp.responseLimitExceeded',
    'Asset metadata cannot fit within the read_assets response limit.',
  );
}

function finalizeReadOutput(
  assets: AssetContent[],
  state: ContinuationState,
): ReadAssetsOutput {
  ...
  if (
    ...
    return readError(
      'mcp.cursorLimitExceeded',
      'The read_assets continuation cursor exceeds its size limit. Read fewer Assets at a time.',
    );
  }
  ...
  let output: ReadAssetsOutput = { ...base, responseBytes: 0 };
  for (let attempt = 0; attempt < 8; attempt += 1) {
    const responseBytes = serializedToolResultBytes(READ_ASSETS_TOOL_NAME, output);
    if (responseBytes === output.responseBytes) return output;
    output = { ...base, responseBytes };
  }
  return output;
}

function finalizedResponseBytes(
  assets: AssetContent[],
  state: ContinuationState,
): number {
...
function fileContent(pathValue: string, content: Buffer): AssetContent['files'][number] {
  return {
    path: pathValue,
...
function advanceAsset(state: ContinuationState): ContinuationState {
  return {
    ...state,
...
function utf8SliceEnd(buffer: Buffer, requestedEnd: number): number {
  if (requestedEnd >= buffer.byteLength) return buffer.byteLength;
  let end = Math.max(0, requestedEnd);
  while (end > 0 && !isUtf8Boundary(buffer, end)) end -= 1;
  return end;
}
...
function decodeContinuation(cursor: string): ContinuationState | undefined {
  try {
...
function readError(code: string, message: string): ReadAssetsOutput {
  return { status: 'error', error: { code, message } };
}

function invalidCursorError(): ReadAssetsOutput {
