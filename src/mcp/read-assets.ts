...
export function readAssets(
  ...
  try {
    ...
    while (state.remaining.length > 0) {
      ...
      const emittedFiles: AssetContent['files'] = [];
...
function utf8SliceEnd(buffer: Buffer, maxBytes: number): number {
  if (maxBytes >= buffer.byteLength) return buffer.byteLength;
...
function decodeContinuation(cursor: string): ContinuationState | undefined {
  try {
    ...
    if (!isRecord(parsed) || !Array.isArray(parsed.remaining)) return undefined;
    const remaining: ContinuationState['remaining'] = [];
