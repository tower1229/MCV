...
import type { PresentationBlock, PresentationRole } from '../presentation/contracts.js';
import { diffLines, fact, status } from '../presentation/builders.js';

export function renderDeployChangeDetailBlocks(changes: DeployChange[]): PresentationBlock[] {
...
export function groupDeployChanges(changes: DeployChange[]): Map<string, DeployChange[]> {
...
export function changeBlock(change: DeployChange): PresentationBlock {
...
function deploymentLabel(kind: DeployChange['deploymentKind']): string {
  switch (kind) {
