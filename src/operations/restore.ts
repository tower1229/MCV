...
export type DeployBackupLayoutKind =
  | 'ordinary-file'
...
export interface DeployBackupFile {
  ...
  linkText?: string;
  layoutKind?: DeployBackupLayoutKind;
}
    ...
    if (!backup) {
      return freezeRestorePlan(failedRestorePlan(
        ...
        scope === 'global'
          ...
          : 'No complete and verified project deployment backup is available for this targetRoot.',
        scope === 'global'
  ...
  try {
    ...
    for (const change of plan.changes) {
      ...
      if (source.nodeKind === 'directory') {
        ...
        if (!sourcePath) throw new Error(`Backup directory is no longer valid for ${change.targetPath}.`);
      ...
      if (source.nodeKind === 'symlink') {
        ...
        const sourcePath = resolveVerifiedBackupSymlink(
          transactionBackup.directory,
          source.backupPath,
          source.linkText,
        );
        if (!sourcePath) throw new Error(`Backup symlink is no longer valid for ${change.targetPath}.`);
        removeFile(change.targetPath);
...
function backupCurrentNode(
  targetPath: string,
  destination: string,
  copyFile: typeof fs.copyFileSync,
): { hash: string; nodeKind: 'file' | 'directory' | 'symlink'; linkText?: string } {
...
function verifyCurrentStateBackup(
  ...
  try {
    ...
    for (const entry of value.files) {
      ...
      if (nodeKind === 'symlink') {
        if (typeof entry.linkText !== 'string') return undefined;
...
function restoreLayoutKind(
  layoutKind: DeployBackupLayoutKind | undefined,
  nodeKind: 'file' | 'directory' | 'symlink',
): RestoreLayoutKind {
  if (layoutKind === 'physical-materialization') return 'physical-package';
  if (layoutKind === 'managed-link-projection' || layoutKind === 'topology-migration') {
    return 'managed-link-projection';
  }
  if (layoutKind === 'copy-projection') return 'copy-projection';
  if (nodeKind === 'symlink') return 'managed-link-projection';
  if (nodeKind === 'directory') return 'copy-projection';
  return 'ordinary-file';
}

export function findLatestVerifiedBackup(
  backupRoot: string,
  filter: { scope: 'project' | 'global'; targetRoot?: string },
...
function backupMatchesRestoreFilter(
  backup: VerifiedDeployBackup,
  filter: { scope: 'project' | 'global'; targetRoot?: string },
): boolean {
  const scope = backup.manifest.scope ?? 'global';
  if (filter.scope === 'global') return scope === 'global';
...
function verifyDeployBackupFile(
  ...
  if (value.layoutKind !== undefined && layoutKind === undefined) return undefined;
  ...
  if (nodeKind === 'directory') {
    ...
    if (!sourcePath || hashDirectoryTree(sourcePath) !== value.beforeHash) return undefined;
    return {
      action,
      originalPath: value.originalPath,
  ...
  if (nodeKind === 'symlink') {
    if (typeof value.linkText !== 'string') return undefined;
...
function parseBackupLayoutKind(value: unknown): DeployBackupLayoutKind | undefined {
...
function resolveVerifiedBackupDirectory(
  directory: string,
  backupPath: string,
): string | undefined {
...
function resolveVerifiedBackupSymlink(
  directory: string,
  backupPath: string,
  linkText: string,
): string | undefined {
  ...
  try {
    const stats = fs.lstatSync(sourcePath);
    if (!stats.isSymbolicLink() || fs.readlinkSync(sourcePath) !== linkText) return undefined;
  } catch {
    return undefined;
  }
  return sourcePath;
