import type { CaptureChange, CapturePlan, CaptureResult } from '../../operations/capture.js';
import {
...
export interface CaptureTuiState {
  status: CaptureTuiStatus;
  ...
  detailChangeId?: string;
  returnStatus?: 'changes' | 'decisions';
  result?: CaptureResult;
  notice?: string;
}

export type CaptureTuiAction =
  ...
  | { type: 'back' }
  | { type: 'apply' }
  | { type: 'applied'; result: CaptureResult }
  | { type: 'regenerating' }
  | { type: 'regenerated'; plan: CapturePlan };

export function createCaptureTuiState(plan: CapturePlan, notice?: string): CaptureTuiState {
...
export function captureTuiReducer(
  state: CaptureTuiState,
  action: CaptureTuiAction,
): CaptureTuiState {
  if (state.status === 'applying' || state.status === 'regenerating') {
    if (action.type === 'applied') return { ...state, status: 'result', result: action.result };
    if (action.type === 'regenerating') return { ...state, status: 'regenerating' };
  ...
  if (action.type === 'regenerated') return createCaptureTuiState(action.plan);
  if (action.type === 'regenerating') return { ...state, status: 'regenerating' };
  if (action.type === 'applied') return { ...state, status: 'result', result: action.result };
...
export function visibleCaptureChanges(state: CaptureTuiState): CaptureChange[] {
  return state.model.plan.changes.filter((change) => !change.decisionGroupId);
}

export function currentCaptureDecisionChoices(state: CaptureTuiState): CaptureChange[] {
  return state.model.decisionGroups[state.decisionGroupIndex]?.choices ?? [];
}

export function captureTuiCanApply(state: CaptureTuiState): boolean {
  return state.status === 'final' && captureReviewComplete(state.model, state.draft);
}

export function captureTuiSummary(state: CaptureTuiState) {
  return summarizeCaptureReview(state.model, state.draft);
}

function move(state: CaptureTuiState, delta: number): CaptureTuiState {
  ...
  if (state.status === 'decisions') {
    return {
      ...state,
      decisionCursor: clamp(
        state.decisionCursor + delta,
        currentCaptureDecisionChoices(state).length,
      ),
    };
  }
...
function toggle(state: CaptureTuiState): CaptureTuiState {
  if (state.status === 'changes') {
  ...
  if (state.status === 'warnings') {
    ...
    return {
      ...state,
      draft: setCaptureWarningConfirmed(
        state.model,
        state.draft,
...
function openDiff(state: CaptureTuiState): CaptureTuiState {
...
function focusedCaptureChange(state: CaptureTuiState): CaptureChange | undefined {
  if (state.status === 'changes') return visibleCaptureChanges(state)[state.changeCursor];
  if (state.status === 'decisions') {
...
function continueReview(state: CaptureTuiState): CaptureTuiState {
  if (state.status === 'changes') {
    if (state.model.decisionGroups.length > 0) return { ...state, status: 'decisions' };
    if (state.model.warnings.length > 0) return { ...state, status: 'warnings' };
  ...
  if (state.status === 'warnings') {
    return state.model.warnings.every((warning) =>
      state.draft.confirmedIssueIds.includes(warning.confirmationId))
      ? { ...state, status: 'final' }
      : state;
  }
  return state;
...
function back(state: CaptureTuiState): CaptureTuiState {
  if (state.status === 'diff') return captureTuiReducer(state, { type: 'closeDiff' });
  if (state.status === 'decisions') {
