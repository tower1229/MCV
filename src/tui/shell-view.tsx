  ...
  if (page.route === 'capture') {
    ...
    if (result.status === 'succeeded') {
      ...
      if (newUnassignedCount > 0) {
        lines.push(
          `New Unassigned: ${newUnassignedCount} asset(s) (${(result.data?.newUnassignedAssetIds ?? []).join(', ')}).`,
        );
      }
      for (const action of result.nextActions) lines.push(`Next: ${action}`);
      const warningStart = lines.length;
      for (const issue of result.issues) lines.push(`Warning: ${issue.message}`);
      return statusPageLines(
        'capture-success',
        'success',
        lines,
        (index) => (index >= warningStart ? 'warning' : undefined),
      );
    }
