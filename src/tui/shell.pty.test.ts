...
describe.skipIf(!fs.existsSync(expectPath))('packaged TUI Shell in a real PTY', () => {
  ...
  it('drives packaged Capture through Diff, decision, warning, and Apply with arrows', async () => {
    const fixturePath = createCaptureWorkflowFixture();
    const outcome = await runExpect([
      'set timeout 5',
      'log_user 1',
      'spawn /bin/zsh -f -c {NO_COLOR=1 "$MCV_TEST_NODE" "$MCV_TEST_SCRIPT"; code=$?; print -r -- EXIT_CODE:$code; exit $code}',
      'expect -exact {Capture · Select Changes}',
      'expect -exact {> [x] ✓ Selected · [modify] settings.json}',
      'send "\\033\\[C"',
      'expect -exact {Capture · Diff}',
      'expect -exact {- old}',
      'send "\\033\\[D"',
      'expect -exact {> [x] ✓ Selected · [modify] settings.json}',
      'send "\\033\\[B"',
      'expect -exact {> [ ] × Destructive · [delete] old.json}',
      'send " "',
      'after 100',
      'send "\\r"',
      'expect -exact {Capture · Resolve Decisions}',
      'send "\\033\\[B"',
      'send " "',
      'after 100',
      'send "\\033\\[C"',
      'expect -exact {Capture · Confirm Apply}',
      'expect -exact {> [ ] ! Warning · Review skipped source.}',
      'send " "',
      'after 100',
      'send "\\033\\[C"',
      'expect -exact {Capture · Confirm Apply}',
      'send "\\r"',
      'expect -exact {Capture · Applying}',
      'send "\\033\\[D"',
      'send " "',
      'send "\\003"',
      'expect -exact {Capture · Result}',
      'expect -exact {× Failed: simulated transaction failure}',
      'send "q"',
      'expect -exact {SELECTED:capture-first,capture-delete,capture-skip}',
      'expect -exact {CONFIRMED:capture.warning}',
      'expect -exact {OUTCOME:completed}',
      'expect -exact {STATUS:failed}',
      'expect -exact {EXIT_CODE:0}',
      'expect eof',
  ...
  it('opens the same Restore workflow from Overview and the restore deep link', async () => {
    ...
    const overview = await runExpect([
      ...
      'expect -exact {> [delete]}',
      'send "\\033\\[C"',
      'expect -exact {Focused Restore detail}',
      'send "\\033\\[D"',
      'expect -exact {Restore Latest Deployment · Review}',
      'send "\\033\\[D"',
      'expect -exact {Status Overview}',
    ...
    const deepLink = await runExpect([
      ...
      'expect -exact {Restore Latest Deployment · Review}',
      'expect -exact {Impact: 1 change(s) to write, 0 change(s) to delete}',
      'send "\\r"',
  ...
  it('keeps no-backup and Restore Conflict states blocked in the PTY', async () => {
    ...
    const conflict = await runExpect([
      ...
      'log_user 1',
      'spawn /bin/zsh -f -c {cd "$MCV_TEST_REPO"; "$MCV_TEST_NODE" "$MCV_TEST_CLI" restore; code=$?; print -r -- EXIT_CODE:$code; exit $code}',
      'expect -exact {× Restore Conflict: Blocked}',
      'expect -exact {Apply disabled}',
      'send "\\r"',
      'after 100',
      'expect -exact {Restore Latest Deployment · Review}',
  ...
  function createCaptureWorkflowFixture(): string {
    const fixturePath = path.join(testRoot, 'capture-workflow.mjs');
    const shellModuleUrl = new URL(
      `file://${path.join(process.cwd(), 'dist', 'tui', 'shell.js')}`,
    ).href;
    const plan = {
      ...
      changes: [
        {
          ...
          change: 'modify',
          defaultSelected: true,
          repositoryPaths: ['ide/codex/native/settings.json'],
          previews: [{
            repositoryPath: 'ide/codex/native/settings.json',
            kind: 'text',
            bytes: 10,
            sha256: 'a'.repeat(64),
            diff: '- old\\n+ new',
        ...
        },
        {
          ...
          repositoryPaths: ['ide/codex/native/old.json'],
          previews: [],
        },
        {
          id: 'capture-choice',
    ...
    const failed = {
      ...
      changes: [],
      issues: [],
      nextActions: ['Generate a new Capture Plan.'],
      error: {
    ...
    fs.writeFileSync(fixturePath, [
      ...
      ');',
      "process.stdout.write(`SELECTED:${selected.join(',')}\\n`);",
      "process.stdout.write(`CONFIRMED:${confirmed.join(',')}\\n`);",
      "process.stdout.write(`OUTCOME:${outcome.reason}\\n`);",
      "process.stdout.write(`STATUS:${outcome.operationStatus}\\n`);",
      '',
    ].join('\n'));
