...
type RepositoryPlanPreviousSurface =
  | { status: 'menu'; cursor: number }
  | { status: 'path'; value: string; menuCursor: number };
type RepositoryPlanContext = RepositoryWorkflowContext & {
  previousSurface: RepositoryPlanPreviousSurface;
};
...
export type ShellAction =
  ...
  | { type: 'restore.move'; delta: number }
  | { type: 'restore.openDetail' }
  | { type: 'restore.back' }
  | { type: 'restore.apply' }
  | { type: 'restore.applied'; result: RestoreResult }
  | { type: 'page.scroll'; delta: number; maximum: number }
...
export function shellReducer(state: ShellState, action: ShellAction): ShellState {
  ...
  switch (action.type) {
    ...
    case 'repository.plan':
      return updateRepositoryWorkflow(state, (workflow) => {
        const previousSurface: RepositoryPlanPreviousSurface =
          workflow.status === 'path'
            ? {
              status: 'path',
    ...
    case 'repository.back':
      ...
      return updateRepositoryWorkflow(state, (workflow) => {
        ...
        const previousSurface = workflow.status === 'path'
          ? { status: 'menu' as const, cursor: workflow.menuCursor }
          : workflow.previousSurface;
...
function isTransactionApplying(state: ShellState): boolean {
...
function isTransactionCompletion(action: ShellAction): boolean {
...
function updateRepositoryWorkflow(
  ...
  if (workflow === state.page.workflow) return state;
  return {
    ...state,
    page: {
      ...state.page,
...
function toggleCaptureWarning(
  ...
  return {
    ...workflow,
    confirmedIssueIds: toggleId(
      workflow.confirmedIssueIds,
...
export function deployVisibleNodes(
  workflow: Pick<DeploySelectionState, 'plan' | 'expandedNodeIds'>,
) {
  return flattenDeploySelectionTree(
    buildDeploySelectionTree(workflow.plan),
    workflow.expandedNodeIds,
  );
}
...
function openDeployNode(
  ...
  if (workflow.expandedNodeIds.includes(node.id)) {
    return {
      ...
      cursor: clampIndex(
        workflow.cursor + 1,
        deployVisibleNodes(workflow).length,
      ),
    };
  }
  return {
...
function collapseDeployNode(
  workflow: DeployWorkflowState,
): DeployWorkflowState {
  if (workflow.status !== 'selection') return workflow;
  const visible = deployVisibleNodes(workflow);
  const focused = visible[workflow.cursor];
  if (!focused) return workflow;
  if (workflow.expandedNodeIds.includes(focused.node.id)) {
    return {
      ...workflow,
      expandedNodeIds: workflow.expandedNodeIds.filter(
        (id) => id !== focused.node.id,
      ),
    };
  }
  if (!focused.parentId) return workflow;
  const parentIndex = visible.findIndex(
    ({ node }) => node.id === focused.parentId,
  );
...
function continueDeployWorkflow(
  ...
  if (workflow.status === 'selection') {
    if (workflow.plan.issues.some((issue) => issue.severity === 'error')) return workflow;
    ...
    if (workflow.plan.issues.some((issue) => issue.severity === 'decisionRequired')) return workflow;
    return {
      status: 'confirmation',
      plan: workflow.plan,
      cursor: workflow.cursor,
  ...
  if (!current || !workflow.decisions[current.id]) return workflow;
