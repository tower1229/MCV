...
export function computeCatalogRevision(assets) {
...
function deriveMcpServers(repositoryPath) {
    ...
    for (const [name, server] of Object.entries(parsed.servers)) {
        ...
        for (const platform of ['macos', 'windows']) {
            ...
            if (!fs.existsSync(absolute))
                continue;
            const overrideParsed = yaml.parse(fs.readFileSync(absolute, 'utf8'));
...
function deriveNativeUnits(repositoryPath) {
    const items = [];
...
function collectSourcePaths(repositoryPath, basePaths) {
    const paths = [];
...
function hashRelativePaths(repositoryPath, relativePaths) {
    const hash = createHash('sha256');
    let sizeBytes = 0;
...
function loadMcpOverrideDocs(repositoryPath) {
    const docs = [];
