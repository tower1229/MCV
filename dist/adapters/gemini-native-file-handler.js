...
class GeminiNativeFileHandler {
            ...
            try {
                ...
                if (Object.keys(native.value).length > 0) {
                    result.files.push({
                        ...
                        content: (0, structured_config_1.stringifyStructuredObject)(native.value, 'json'),
                        ownership: 'native',
                    });
                }
    ...
    async deploy(repositoryPath, context) {
        ...
        if (fs.existsSync(sourcePath)) {
            const parsed = (0, structured_config_1.parseStructuredObject)(fs.readFileSync(sourcePath, 'utf8'), 'json', sourcePath);
            const resolved = (0, variables_1.resolvePortableValue)(parsed, context.variables ?? {}, context.platform ?? process.platform);
            files.push({
                targetPath,
                content: (0, structured_config_1.stringifyStructuredObject)(resolved, 'json'),
            });
        }
