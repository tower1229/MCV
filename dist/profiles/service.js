        ...
        replaceAll(input) {
            return mutate(repositoryPath, input, (document) => {
                ...
                for (const id of Object.keys(input.profiles)) {
                    ...
                    if (id in document.profiles)
                        updated.push(id);
                    else
                        created.push(id);
                }
                document.profiles = Object.fromEntries(Object.entries(input.profiles).map(([id, profile]) => [id, normalizeProfile(profile)]));
            ...
            return mutate(repositoryPath, input, (document) => {
                ...
                for (const mutation of input.mutations) {
                    if (mutation.operation === 'delete') {
                        ...
                        if (!deleted.includes(mutation.id))
                    ...
                    if (!existing) {
                        ...
                        if (updatedIndex >= 0)
                            updated.splice(updatedIndex, 1);
                        if (!created.includes(mutation.id))
...
function mutateWhileLocked(repositoryPath, input, prepare) {
    ...
    if (input.expectedProfilesRevision !== profilesRevision
    ...
    for (const [profileId, profile] of Object.entries(working.profiles)) {
        for (const assetId of profile.assets) {
            if (!isValidAssetId(assetId)) {
                return rejectResult(profilesRevision, catalog.revision, 'profile.invalidAssetId', `Profile ${profileId} references invalid Asset ID ${assetId}.`);
            }
            if (!catalogIds.has(assetId)) {
                return rejectResult(profilesRevision, catalog.revision, 'profile.unknownAsset', `Profile ${profileId} references unknown Asset ${assetId}.`);
            }
        }
    }
    if (!(GLOBAL_PROFILE_ID in working.profiles)) {
        return rejectResult(profilesRevision, catalog.revision, 'profile.globalRequired', 'The built-in global Profile must remain present.');
    }
...
function buildDiff(beforeAssets, afterProfiles, deleted) {
    const diff = {};
    for (const [id, profile] of Object.entries(afterProfiles)) {
        const before = beforeAssets[id] ?? new Set();
...
function rejectResult(profilesRevision, catalogRevision, code, message) {
    return {
        status: 'rejected',
        created: [],
        updated: [],
