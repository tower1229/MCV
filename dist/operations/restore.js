...
export function applyRestorePlan(context, plan, selection, options = {}) {
        ...
        if (stateCommitAttempted) {
            try {
                if (previousStateContent)
                    atomicWriteFile(statePath, previousStateContent);
                else
...
function sameRestoreSnapshot(context, plan, backup) {
    try {
        if (readState(context).repositoryPath !== (plan.repositoryPath ?? undefined))
            return false;
        if (plan.repositoryPath)
            readManifest(plan.repositoryPath);
        if (!plan.backup
