...
import { acquireOperationLock, deployOperationLockResource, OperationLockBusyError, releaseOperationLock, } from '../utils/operation-lock.js';
...
import { managedReceiptPath, parseManagedReceipt, readManagedReceipt, serializeManagedReceipt, } from '../core/managed-receipt.js';
...
export const PROJECT_SCOPE_UNSUPPORTED_CODE = 'deploy.projectScopeUnsupported';
...
async function buildDeployPlan(context, repositoryPath, operationId, mutations, request) {
    ...
    for (const [targetPath, inventoryEntry] of Object.entries(managedInventory)) {
        ...
        const containsDesiredPath = [...desiredPaths].some((desiredPath) => isPathWithinRoot(resolvedTarget, desiredPath));
        if (containsDesiredPath || !deployPathExists(targetPath))
            continue;
...
function addExternalLinkReplacementDecisions(facts, outcomes, desired, useSymlinks, context, changes, mutations, issues) {
    const decisions = [];
    for (const fact of facts.filter((entry) => entry.severity === 'decisionRequired')) {
        const replacementChangeIds = [];
        for (const outcome of outcomes.filter((entry) => entry.factId === fact.id && entry.owner === 'ide')) {
            if (!outcome.ide || !outcome.surface)
                continue;
            for (const linkPath of outcome.linkPaths) {
                const packageFiles = desired.filter((file) => file.capability === 'skills'
                    && file.owner === 'ide'
                ...
                const id = selectionId(canonicalSkillTargetKey(outcome), 'skills', `${linkPath}\0external-link-replacement`);
                ...
                const dependsOnChangeIds = useSymlinks
                    ? changes.filter((change) => change.deploymentKind === 'physical-materialization'
                ...
                changes.push({
                    ...
                    preview: useSymlinks
                        ...
                        : {
                            targetPath: linkPath,
                            kind: 'package',
                            files: packageFiles.map((file) => preview(file.targetPath, canonicalSkillTargetKey(outcome), 'skills', toBuffer(file.content), fs.existsSync(file.targetPath) ? fs.readFileSync(file.targetPath) : undefined, issues)).filter((item) => item.kind === 'text' || item.kind === 'binary'),
                        },
                });
...
export async function applyDeployPlan(context, plan, selection, options = {}) {
    ...
    try {
        return await applyDeployPlanWhileLocked(context, plan, repositoryPath, selection, options, active, selected, selectedIds);
    }
    finally {
        releaseOperationLock(lock);
    }
}
async function applyDeployPlanWhileLocked(context, plan, repositoryPath, selection, options, active, selected, selectedIds) {
    let freshPlan;
    try {
        freshPlan = await buildDeployPlan(context, repositoryPath, plan.operationId, new Map(), deployRequestFromPlan(plan));
    }
    catch {
        activeDeployPlans.delete(plan);
        return failedDeployResult(plan.repositoryPath, stalePlanError(), undefined, deployContextFromPlan(plan));
...
function deployBlockingIssues(plan, selection, options) {
    ...
    const warnings = plan.issues.filter((issue) => issue.severity === 'warning' && !confirmed.has(issue.confirmationId));
...
function prepareDeployWrites(changes, mutations) {
    ...
    return [...grouped].map(([targetPath, targetChanges]) => {
        ...
        const order = { write: 0, delete: 1, link: 2, 'migrate-link': 2, 'replace-directory': 2 };
...
function freezeDeployPlan(plan) {
    ...
    for (const fact of plan.linkFacts) {
        ...
        if (fact.resolvedPaths)
            Object.freeze(fact.resolvedPaths);
...
function updateSelectedManagedReceipt(plan, selected, selectedIds, mutations, update) {
    if (plan.scope !== 'project')
        return;
    const receiptChange = plan.changes.find((change) => change.name === 'Managed Receipt'
        && change.targetPath === managedReceiptPath(plan.targetRoot));
    ...
    let receipt;
    try {
        receipt = parseManagedReceipt(JSON.parse(receiptMutation.content.toString('utf8')));
    }
    catch {
        return;
    }
    ...
    selected.add(receiptChange.id);
    if (!selectedIds.includes(receiptChange.id))
        selectedIds.push(receiptChange.id);
...
function applyProjectPruneReceiptDecisions(plan, selected, selectedIds, mutations) {
    updateSelectedManagedReceipt(plan, selected, selectedIds, mutations, (receipt) => {
        ...
        for (const change of plan.changes) {
            ...
            if (!mutation?.receiptKey || !mutation.receiptEntry || selected.has(change.id))
                continue;
...
function appendProjectMcpPlan(targetRoot, mcpServers, mcpOverrides, manifest, receipt, receiptEntries, changes, decisions, mutations, issues) {
    ...
    for (const target of targets) {
        ...
        for (const name of serverNames) {
            const native = toNativeMcpServers({ [name]: mcpServers[name] }, target.surface, mcpOverrides[target.surface]);
            const desired = native[name];
            if (!isRecord(desired))
                continue;
            let projection;
            try {
                projection = projectMcpServer(targetRoot, target, { assetId: `mcp:${name}`, name, desired }, receipt);
            }
            catch (error) {
                issues.push({
                    severity: 'error',
        ...
        if (Object.keys(serversToWrite).length === 0)
            continue;
        try {
            assertPathContainedInProjectRoot(targetRoot, targetPath);
        }
        ...
        const nextContent = Buffer.from(overlayProjectMcpFile(previous?.toString('utf8'), target, serversToWrite), 'utf8');
        if (previous?.equals(nextContent))
            continue;
...
function applyProjectMcpOverlayDecisions(plan, selection, selected, selectedIds, mutations) {
    if (plan.scope !== 'project')
        return;
    const preservedByChange = new Map();
    for (const decision of plan.decisions) {
        if (decision.kind !== 'project-mcp-divergence')
            continue;
        const choice = selection.decisions?.[decision.id];
    ...
    updateSelectedManagedReceipt(plan, selected, selectedIds, mutations, (receipt) => {
        ...
        for (const decision of plan.decisions) {
            if (decision.kind !== 'project-mcp-divergence')
                continue;
            if (selection.decisions?.[decision.id] !== 'preserve-external')
                continue;
