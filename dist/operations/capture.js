...
async function buildCapturePlan(context, repositoryPath, operationId, mutations) {
    ...
    const issues = captured.flatMap(({ result }, resultIndex) => result.warnings.map((warning, warningIndex) => ({
        severity: 'warning',
...
function hashSourcePaths(sourcePaths) {
    ...
    for (const sourcePath of unique) {
        ...
        if (sourcePath.startsWith('skill-topology:'))
            continue;
...
function addSkillChanges(repositoryPath, packages, changes, issues, plannedRepositoryPaths, mutations) {
    for (const [name, copies] of packages) {
        ...
        const change = {
            ...
            contributingProjections: [...selected.projections].sort((left, right) => left.surface.localeCompare(right.surface)
...
function uniqueSkillCopies(copies) {
    ...
    for (const copy of copies) {
        ...
        for (const projection of copy.projections) {
            if (!existing.projections.some((entry) => path.resolve(entry.projectionPath) === path.resolve(projection.projectionPath))) {
