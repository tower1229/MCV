...
- Rules、同名 Skill 或同名 MCP 核心定义冲突时逐项选择权威来源；非交互安全模式不会替用户猜测。
...
新设备先执行 `mcv bind <path>`。普通命令校验本机绑定的 repository ID，不会因为当前目录恰好存在另一个 `mcv.yaml` 就越过绑定。
...
- `status` 报告绑定、Git dirty、IDE/Surface、缺失环境变量、部署基线漂移和最近操作。
- `restore` 先验证文件在 deploy 后没有额外变化，再备份当前状态并事务恢复；deploy 新增的文件会被删除。
