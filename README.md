...
- `restore --dry-run` 只选择最近一次完整且内容可验证的 Deploy backup，展示备份时间、将恢复或删除的文件，并以独立的 Restore Conflict 阻止覆盖部署后的新变化；实际 Restore 仍会先备份当前状态再事务恢复。
...
mcv restore    通过 --dry-run 审阅最近一次完整 Deploy backup；支持 --json
```
